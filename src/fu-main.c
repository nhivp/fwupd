...
fu_main_private_free (FuMainPrivate *priv)
{
	...
	if (priv->memory_monitor != NULL)
		g_object_unref (self->memory_monitor);
#endif
	g_free (priv);
}
...
main (int argc, char *argv[])
{
	...
	self->memory_monitor = g_memory_monitor_dup_default ();
	g_signal_connect (G_OBJECT (self->memory_monitor), "low-memory-warning",
			  G_CALLBACK (fu_main_memory_monitor_warning_cb), self);
#endif
