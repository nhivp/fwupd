...
fu_vli_pd_parade_device_to_string (FuDevice *device, guint idt, GString *str)
{
	...
	fu_common_string_append_kx (str, idt, "PageAddr", self->page_addr);
	fu_common_string_append_kx (str, idt, "Page7Addr", self->page7_addr);
}
				  ...
				  GError **error)
{
	...
	/* page2_address = 0x14/0x94, page7_address = 0x1E/0x9E --
	 * but VL103 FW only Use bits[7:1], so divide by 2 */
	value = ((guint16) reg_offset << 8)| (page_addr >> 1);
	if (!g_usb_device_control_transfer (fu_usb_device_get_dev (FU_USB_DEVICE (self)),
					    ...
					    NULL, error)) {
		g_prefix_error (error, "failed to read 0x%x:0x%x: ", page_addr, reg_offset);
		return FALSE;
	}
	return TRUE;
}
				   ...
				   GError **error)
{
	...
	if (!g_usb_device_control_transfer (fu_usb_device_get_dev (FU_USB_DEVICE (self)),
					    ...
					    FU_VLI_DEVICE_TIMEOUT,
					    NULL, error)) {
		g_prefix_error (error, "failed to write 0x%x:0x%x: ", page_addr, reg_offset);
		return FALSE;
	}
	return TRUE;
}
...
{
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xBC, 0xC0, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xBC, 0x40, error))
		return FALSE;
	return TRUE;
}
...
{
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x8E, addr >> 8, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x8F, addr & 0xff, error))
		return FALSE;
	return TRUE;
}
...
fu_vli_pd_parade_device_read_fw_ver (FuVliPdParadeDevice *self, GError **error)
{
	...
	if (!fu_vli_pd_parade_device_set_offset (self, 0x0, error))
		return FALSE;
	g_usleep (1000 * 10);
	if (!fu_vli_pd_parade_device_i2c_read (self, self->page7_addr, 0x02, buf, 0x1, error))
		return FALSE;
	...
	if (!fu_vli_pd_parade_device_set_offset (self, 0x5000 | buf[0], error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_read (self, self->page7_addr, 0x00, buf, sizeof(buf), error))
		return FALSE;

	/* format as a hex triplet */
	version_str = g_strdup_printf ("%02X.%02X.%02X", buf[0], buf[1], buf[2]);
...
{
	return fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xB3,
						  val ? 0x10 : 0x00, error);
}
...
fu_vli_pd_parade_device_write_enable (FuVliPdParadeDevice *self, GError **error)
{
	...
	if (!fu_vli_pd_parade_device_set_wp (self, TRUE, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, 0x06, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x92, 0x00, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x93, 0x05, error))
		return FALSE;
...
{
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xDA, 0x00, error))
		return FALSE;
	return TRUE;
}
...
fu_vli_pd_parade_device_write_status (FuVliPdParadeDevice *self, guint8 target_status, GError **error)
{
	...
	if (!fu_vli_pd_parade_device_set_wp (self, TRUE, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, 0x01, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, target_status, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x92, 0x01, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x93, 0x05, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_set_wp (self, FALSE, error))
...
fu_vli_pd_parade_device_wait_ready (FuVliPdParadeDevice *self, GError **error)
{
	...
	/* Wait_For_SPI_ROM_Until_Not_Busy */
	for (guint wait_cnt1 = 0; wait_cnt1 < limit; wait_cnt1++) {
		buf = 0xFF;
		if (!fu_vli_pd_parade_device_i2c_read (self, self->page_addr,
						       0x9E, &buf, sizeof(buf),
						       error))
			return FALSE;
		/* Busy_Status, bit[1,0]:Byte_Program, bit[3,2]:Sector_Erase, bit[5,4]:Chip_Erase */
		if ((buf & 0x0C) == 0) {
			ret = TRUE;
			break;
		}
	...
	/* Wait_SPI_Until_Ststus_Bit_0_Clear */
	ret = FALSE;
	for (guint wait_cnt1 = 0; wait_cnt1 < limit; wait_cnt1++) {
		gboolean ret2 = FALSE;

		/* SPI_RSTS_05, Len_01, Trigger_Read */
		if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, 0x05, error))
			return FALSE;
		if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x92, 0x00, error))
			return FALSE;
		if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x93, 0x01, error))
			return FALSE;
...
fu_vli_pd_parade_device_sector_erase (FuVliPdParadeDevice *self, guint16 addr, GError **error)
{
	...
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, 0x20, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, addr >> 8, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, addr & 0xff, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x90, 0x00, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x92, 0x03, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x93, 0x05, error))
		return FALSE;
	return TRUE;
...
{
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xDA, 0xAA, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xDA, 0x55, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xDA, 0x50, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xDA, 0x41, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xDA, 0x52, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0xDA, 0x44, error))
		return FALSE;
	return TRUE;
}
...
fu_vli_pd_parade_device_block_erase (FuVliPdParadeDevice *self, guint8 block_idx, GError **error)
{
	...
	for (guint idx = 0; idx < 0x100; idx += 0x10){
		...
		if (!fu_vli_pd_parade_device_set_offset (self, (block_idx << 8) | idx, error))
			return FALSE;
		if (!fu_vli_pd_parade_device_i2c_read (self, self->page7_addr, 0, buf, 0x20, error))
			return FALSE;
				    ...
				    GError **error)
{
	for (guint idx = 0; idx < 0x100; idx++){
		...
		for (guint idx2 = 0; idx2 < 0x100; idx2 += 0x20){
			...
			if (!fu_vli_pd_parade_device_i2c_read (self, self->page7_addr, idx2, buf + buf_offset, 0x20, error))
				return FALSE;
		}
	}
	return TRUE;
					...
					GError **error)
{
	...
	if (!fu_vli_pd_parade_device_enable_mapping (self, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x82, 0x20, error))
		return FALSE; /* Reset_CLT2SPI_Interface */
	g_usleep (1000 *  100 );
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page_addr, 0x82, 0x00, error))
		return FALSE;
	...
	if (!fu_vli_pd_parade_device_set_offset (self, 0x0, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page7_addr, 0x00, 0x55, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page7_addr, 0x01, 0xAA, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page7_addr, 0x02, (guint8) block_idx_tmp, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_write (self, self->page7_addr, 0x03, (guint8) (0x01 - block_idx_tmp), error))
		return FALSE;
	if (!fu_vli_pd_parade_device_write_disable (self, error))
		return FALSE;
	...
	if (!fu_vli_pd_parade_device_set_offset (self, 0x0, error))
		return FALSE;
	if (!fu_vli_pd_parade_device_i2c_read (self, self->page7_addr, 0, buf, sizeof(buf), error))
		return FALSE;
	if (buf[0] != 0x55 ||
	    buf[1] != 0xAA ||
	    buf[2] != block_idx_tmp ||
